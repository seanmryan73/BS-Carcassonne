...
- **Service worker cache version must be bumped** — when any cached asset changes, increment the cache name in `sw.js` (currently `carc-v16`). Forgetting this causes stale assets for installed PWA users.
